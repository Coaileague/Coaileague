...
- **CoAI Twin Mascot (Dec 2, 2025):** Your unique emotional twin-star interactive mascot rendered as draggable 200px (expanded 300px) interactive bubble. Globally visible on ALL pages (public and authenticated). Features: state-reactive behaviors, persistent positioning via localStorage, drag controls with smooth positioning, expand/minimize/reset buttons, and tooltip guidance. CoAI Twin is your exclusive platform companion—uniquely designed for CoAIleague. **Universal Configuration:** All mascot settings are centralized in `client/src/config/mascotConfig.ts` - edit this single file to change mascot behavior platform-wide (sizes, z-index, hidden routes, colors, animation settings). NO page-by-page edits required.
