...
## Recent Changes
-   **Two Universal Chat Clients Architecture** (November 12, 2025): Consolidated platform from 3 separate chat implementations to exactly TWO universal clients as required: `/chat` (HelpDesk desktop) and `/mobile-chat` (HelpDeskMobile). Deleted PremiumMobileChat.tsx entirely and removed all `/premium-chat` routes from App.tsx. Integrated ALL premium features (typing indicators via TypingIndicator component, read receipts display, participant management via ParticipantDrawer, quick-insert macros via MacrosDrawer, file uploads with preview) directly into HelpDeskMobile (`/mobile-chat`). Both clients now share identical feature sets with responsive UI adaptations. Enhanced useChatroomWebSocket hook exposes unified API supporting both legacy moderation and premium features. Navigation protection applied uniformly to both clients using reusable useNavigationProtection hook. Component architecture: four reusable chat components (MessageBubble, TypingIndicator, ParticipantDrawer, MacrosDrawer) following AutoForce emerald theme for maintainability. Backend infrastructure unchanged: automated WebSocket cleanup (5-minute cron), secure typing indicator APIs, workspace-scoped chat macros all operational.
-   **Universal Chat Navigation Protection** (November 12, 2025): Implemented comprehensive navigation protection across ALL chatroom pages to prevent accidental disconnects from live support. Created reusable `useNavigationProtection` hook with three-layer protection: beforeunload warnings (refresh/close/external navigation), popstate handling (back button with proper history management), and in-app route interception (Wouter sidebar/link clicks). Protection activates when connected or conversation has messages. Fixed history pollution bugs and implemented proper user confirmation flow with single-press exit. Also updated browserslist database from 1.0.30001677 to 1.0.30001754 for latest browser compatibility data.
...
-   **Navigation Protection System**: Reusable `useNavigationProtection` hook (client/src/hooks/use-navigation-protection.ts) provides three-layer protection against accidental navigation from active chat sessions: Layer 1 (beforeunload) warns on refresh/close/external navigation, Layer 2 (popstate) handles back button with proper history management and sentinel states, Layer 3 (useLocation) intercepts in-app route changes. Uses ref-based state management to prevent history pollution and stale closures. When user confirms exit after warning, temporarily disables guard and programmatically navigates back to skip duplicate sentinel entry, ensuring single-press exit.
