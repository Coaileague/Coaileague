...
- **SubagentConfidenceMonitor:** Trinity AI Brain service that monitors subagent performance and maintains persistent confidence scores. Tracks per-subagent trust levels based on historical success rate (40%), execution time (15%), error recovery (10%), and base score (15%). Calculates org-level automation readiness for graduation from HAND_HELD → GRADUATED → FULL_AUTOMATION modes. Generates AI-powered optimization suggestions for workflow enhancements, fixes, and performance improvements. REST API: `/api/ai-brain/confidence/org`, `/confidence/subagent/:id`, `/confidence/graduation`, `/confidence/graduate`, `/confidence/suggestions`, `/confidence/trinity-summary`.
...
- **Trinity Org Intelligence:** Real-time org awareness system providing Trinity with live business metrics. `gatherOrgIntelligence()` aggregates automation readiness scores (via SubagentConfidenceMonitor), workboard stats, notification summaries, and invoice status. Data flows through `TrinityContext` to frontend `ThoughtManager` for data-driven dialogue prioritization (priority insights > data-driven > persona pools). Trinity sessions are org-isolated with persistent knowledge, while support roles share universal RBAC-filtered Trinity access.
- **ThoughtManager Automation Integration:** Real-time WebSocket subscription for automation events (automation_event, fast_mode_result, graduation_milestone). Methods: `ingestAutomationEvent()` for job completion/failure notifications, `ingestFastModeResult()` for FAST mode SLA tracking with credit refund calculations, `ingestGraduationMilestone()` for automation level progression celebrations, and `ingestOrgInsights()` for priority insight injection into thought queue.
