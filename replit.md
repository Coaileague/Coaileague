...
-   **Universal Labeled Navigation Menu**: All navigation buttons (Tutorial, Search, Help, Feedback, What's New) display both icons and text labels universally across desktop AppSidebar and mobile UniversalNavHeader. Components updated: HelpDropdown, FeedbackWidget, WhatsNewBadge changed from icon-only (`size="icon"`) to labeled buttons (`size="sm"` with `justify-start gap-2 h-9 w-full`). Layout uses grid (cols-2) for Tutorial/Search and Help/Feedback pairs, full width for What's New with badge count positioned via ml-auto. Quick Tools section added to mobile UniversalNavHeader Sheet menu with PlanBadge, Tutorial, Search, Help, Feedback, and What's New buttons. Desktop and mobile now feature identical labeled navigation controls for consistent UX.
-   **Organization Registration Flow**: Transaction-safe employee record creation during organization signup. Registration creates User → Workspace → Employee (org_owner role) with employee INSERT separated from external ID attachment to prevent rollback on external ID failures. External identifiers can be attached post-registration via retry mechanism. Employee records always persist even if attachEmployeeExternalId fails. Verified via e2e tests: registration succeeds (201), employee record exists (GET /api/employees/me returns 200 with workspaceRole "org_owner"), login works, workspace identity badges display correctly.
