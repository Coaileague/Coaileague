...
// ============================================================================
// TRINITY COMMAND REQUESTS - AI-Driven Human Governance System (1% Approval)
// ============================================================================
// Trinity pushes command/approval requests to users for actions requiring human oversight.
// This replaces the old Universal Notification System with a focused, action-oriented approach.

export const commandRequestStatusEnum = pgEnum('command_request_status', [
  'pending',      // Awaiting user response
  'approved',     // User approved the action
  'rejected',     // User rejected the action
  'expired',      // Request timed out without response
  'cancelled',    // System cancelled the request
  'executing',    // Approval received, action being executed
  'completed',    // Action completed successfully
  'failed',       // Action failed after approval
]);

export const commandRequestPriorityEnum = pgEnum('command_request_priority', [
  'critical',   // Requires immediate attention (red)
  'high',       // Important, respond soon (orange)
  'medium',     // Standard priority (yellow)
  'low',        // When convenient (blue)
]);

export const commandRequestCategoryEnum = pgEnum('command_request_category', [
  'hotpatch',       // Code/config change approval
  'scheduling',     // Schedule approval/conflict resolution
  'payroll',        // Payroll processing approval
  'billing',        // Invoice/billing approval
  'compliance',     // Compliance-related approval
  'security',       // Security-related approval
  'integration',    // Third-party integration action
  'maintenance',    // System maintenance approval
  'escalation',     // Human escalation from AI
  'general',        // General approval request
]);

export const trinityCommandRequests = pgTable("trinity_command_requests", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  
  workspaceId: varchar("workspace_id").references(() => workspaces.id, { onDelete: 'cascade' }),
  
  targetUserId: varchar("target_user_id").references(() => users.id, { onDelete: 'cascade' }),
  targetRole: varchar("target_role"), // Optional: target by role instead of specific user
  
  category: commandRequestCategoryEnum("category").notNull().default('general'),
  priority: commandRequestPriorityEnum("priority").notNull().default('medium'),
  status: commandRequestStatusEnum("status").notNull().default('pending'),
  
  title: varchar("title", { length: 255 }).notNull(),
  description: text("description").notNull(),
  
  actionId: varchar("action_id"), // AI Brain action to execute on approval
  ...
  createdBy: varchar("created_by"), // 'trinity' or user ID
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
}, (table) => ({
  workspaceIdx: index("tcr_workspace_idx").on(table.workspaceId),
  targetUserIdx: index("tcr_target_user_idx").on(table.targetUserId),
  targetRoleIdx: index("tcr_target_role_idx").on(table.targetRole),
  statusIdx: index("tcr_status_idx").on(table.status),
  priorityIdx: index("tcr_priority_idx").on(table.priority),
  categoryIdx: index("tcr_category_idx").on(table.category),
  createdAtIdx: index("tcr_created_at_idx").on(table.createdAt),
  expiresAtIdx: index("tcr_expires_at_idx").on(table.expiresAt),
  pendingUserIdx: index("tcr_pending_user_idx").on(table.targetUserId, table.status),
}));

export const insertTrinityCommandRequestSchema = createInsertSchema(trinityCommandRequests).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

export type InsertTrinityCommandRequest = z.infer<typeof insertTrinityCommandRequestSchema>;
export type TrinityCommandRequest = typeof trinityCommandRequests.$inferSelect;
