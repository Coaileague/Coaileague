...
// ============================================================================
// DATABASE TABLE - Persistent Task Storage
// ============================================================================

export const aiBrainTasks = pgTable("ai_brain_tasks", {
  id: varchar("id", { length: 100 }).primaryKey(),
  workspaceId: varchar("workspace_id", { length: 100 }).notNull(),
  userId: varchar("user_id", { length: 100 }).notNull(),
  
  // Intent (stored as JSON)
  intent: jsonb("intent").notNull().$type<TrinityIntent>(),
  
  // Status and Phase
  status: trinityTaskStatusEnum("status").notNull().default('pending'),
  phase: trinityTaskPhaseEnum("phase").notNull().default('intake'),
  
  // Plan details
  planId: varchar("plan_id", { length: 100 }),
  framework: varchar("framework", { length: 50 }),
  steps: jsonb("steps").default([]).$type<TrinityTaskStep[]>(),
  currentStepIndex: integer("current_step_index").default(0),
  
  // Confidence
  overallConfidence: integer("overall_confidence").default(100), // Stored as 0-100
  stepConfidences: jsonb("step_confidences").default({}).$type<Record<string, number>>(),
  
  // Reflections
  reflections: jsonb("reflections").default([]).$type<TrinityReflection[]>(),
  maxReflectionCycles: integer("max_reflection_cycles").default(3),
  
  // State history
  stateHistory: jsonb("state_history").default([]).$type<TrinityStateTransition[]>(),
  
  // Output
  output: jsonb("output").$type<TrinityTaskOutput>(),
  ...
  actualCreditsUsed: integer("actual_credits_used").default(0),
  
  // Relationships
  parentTaskId: varchar("parent_task_id", { length: 100 }),
  childTaskIds: jsonb("child_task_ids").default([]).$type<string[]>(),
  
  // Subagent routing
  primarySubagent: varchar("primary_subagent", { length: 100 }),
  involvedSubagents: jsonb("involved_subagents").default([]).$type<string[]>(),
  
  // Risk level for overall task
  riskLevel: trinityRiskLevelEnum("risk_level").default('low'),
  
  // Error tracking
  lastError: text("last_error"),
  errorCount: integer("error_count").default(0),
}, (table) => [
  index("ai_brain_tasks_workspace_idx").on(table.workspaceId),
  index("ai_brain_tasks_user_idx").on(table.userId),
  index("ai_brain_tasks_status_idx").on(table.status),
  index("ai_brain_tasks_phase_idx").on(table.phase),
  index("ai_brain_tasks_created_idx").on(table.createdAt),
  index("ai_brain_tasks_parent_idx").on(table.parentTaskId),
  index("ai_brain_tasks_subagent_idx").on(table.primarySubagent),
]);

// Insert schema for type-safe insertions
export const insertAiBrainTaskSchema = createInsertSchema(aiBrainTasks).omit({
  createdAt: true,
});
export type InsertAiBrainTask = z.infer<typeof insertAiBrainTaskSchema>;
export type AiBrainTask = typeof aiBrainTasks.$inferSelect;
