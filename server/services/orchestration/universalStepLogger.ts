...
class UniversalStepLogger {
  ...
  async stagePayload(orchestrationId: string, payload: Record<string, unknown>): Promise<void> {
    ...
    }).catch((err: unknown) => log.warn('[UniversalStepLogger] publish orchestration_pending_approval failed:', (err instanceof Error ? err.message : String(err))));
  }
  ...
  async rejectPayload(orchestrationId: string, rejectedBy: string, reason: string): Promise<void> {
    ...
    }).catch((err: unknown) => log.warn('[UniversalStepLogger] publish orchestration_rejected failed:', (err instanceof Error ? err.message : String(err))));
  }
  ...
  private async triggerUpsell(context: OrchestrationContext, check: SubscriptionCheckResult): Promise<void> {
    ...
    }).catch((err: unknown) => log.warn('[UniversalStepLogger] publish upsell_triggered failed:', (err instanceof Error ? err.message : String(err))));
