    ...
    try {
      ...
      const payrollData = await db
        .select({
          ...
          overtime: sql<string>`COALESCE(SUM(${(payrollEntries as Record<string,unknown>).overtimePay}), 0)`,
          regular: sql<string>`COALESCE(SUM(${(payrollEntries as Record<string,unknown>).regularPay}), 0)`,
        })
        .from(payrollEntries)
