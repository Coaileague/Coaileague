...
export async function seedStateKnowledgeBase(): Promise<void> {
  ...
  for (const state of STATE_REGULATORY_KNOWLEDGE) {
    ...
    const data = {
      ...
      licenseTypes: state.licenseTypes as unknown,
      keyStatutes: state.keyStatutes as unknown,
      minimumInsuranceCoverage: state.minimumInsuranceCoverage as unknown,
      uniformRequirement: state.uniformRequirement,
      vehicleMarkingRequirement: state.vehicleMarkingRequirement,
      hardBlockRules: state.hardBlockRules as unknown,
...
export async function getStateConfig(stateCode: string): Promise<StateRegulatoryConfig | null> {
  ...
  return {
    ...
    auditorEmailDomain: (row as Record<string, unknown>).auditorEmailDomain ?? '',
    licenseTypes: ((row as Record<string, unknown>).licenseTypes as unknown[]) ?? [],
    keyStatutes: ((row as Record<string, unknown>).keyStatutes as unknown[]) ?? [],
    minimumInsuranceCoverage: ((row as Record<string, unknown>).minimumInsuranceCoverage as unknown[]) ?? [],
    uniformRequirement: (row as Record<string, unknown>).uniformRequirement ?? '',
    vehicleMarkingRequirement: (row as Record<string, unknown>).vehicleMarkingRequirement ?? '',
    hardBlockRules: ((row as Record<string, unknown>).hardBlockRules as unknown[]) ?? [],
