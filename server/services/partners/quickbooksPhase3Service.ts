...
export const qbPhase3 = {
  async sync(workspaceId: string, entityType: 'invoices' | 'employees' | 'payroll' = 'invoices') {
    try {
      ...
      log.error(`[QB Phase3] Sync failed: ${(err instanceof Error ? err.message : String(err))}`);
      return { success: false, entityType, itemCount: 0, error: (err instanceof Error ? err.message : String(err)) };
    }
  },
