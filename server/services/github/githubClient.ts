...
export const githubClient = {
  async request<T = unknown>(path: string, method = 'GET', body?: unknown): Promise<T | null> {
    ...
    } catch (err: unknown) { log.warn(`[GitHub] Request failed: ${(err instanceof Error ? err.message : String(err))}`); return null; }
  },
  ...
  async recordScan(findings: unknown[], scanType = 'platform') {
    ...
    } catch (err: unknown) { log.warn(`[GitHub] Record scan failed: ${(err instanceof Error ? err.message : String(err))}`); }
  },
};
