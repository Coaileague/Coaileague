...
export async function createSchedulePublishedNotification(
  ...
  await createNotification({
    ...
    idempotencyKey: `schedule_published-${Math.floor(Date.now() / (6 * 60 * 60 * 1000))}-${params.userId}`,
...
export async function createInvoiceCreatedNotification(
  ...
  await createNotification({
    ...
    idempotencyKey: `invoice_created-${Math.floor(Date.now() / (6 * 60 * 60 * 1000))}-${params.userId}`,
