...
class LoneWorkerService {
  ...
  async escalateMissedCheckIn(workspaceId: string, sessionId: string, missedCount: number) {
    ...
    await platformEventBus.publish({
      ...
      description: `${(s as Record<string, unknown>).employee_name || 'Officer'} has missed ${missedCount} check-in(s). Escalating to ${level}.`,
      workspaceId,
    ...
    log.warn(`Lone worker missed check-in escalated to ${level}: ${(s as Record<string, unknown>).employee_name || 'officer'}, missed=${missedCount}`);
  }
