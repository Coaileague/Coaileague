  ...
  async start(): Promise<void> {
    ...
    this.expiryCheckInterval = setInterval(async () => {
      try {
        ...
        log.warn('Check cycle failed (will retry)', { error: (error instanceof Error ? error.message : String(error)) || 'unknown' });
      }
    ...
    return automationOrchestration.executeAutomation(
      ...
      async (ctx) => {
        ...
        const requestData: InsertShiftCoverageRequest = {
          ...
          originalEmployeeId: input.originalEmployeeId || shift.employeeId || null,
