...
async function safeNumber(query: string, params: unknown[] = []): Promise<number> {
  try {
    ...
    log.warn(`[telemetry] query failed: ${(err instanceof Error ? err.message : String(err))}`);
    return 0;
  }
}
...
async function chaosSemanticAiTriage(workspaceId: string): Promise<ChaosTestRow> {
  ...
  try {
    ...
    const mod: Record<string, unknown> = await dynImport('../../trinity/staffingIntake').catch(() => null);
...
export async function runAllChaosTests(workspaceId: string): Promise<{
  ...
  try {
    ...
    log.warn(`[chaos] plaid sandbox check skipped: ${(err instanceof Error ? err.message : String(err))}`);
  }
