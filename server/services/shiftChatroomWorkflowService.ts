...
class ShiftChatroomWorkflowService {
  ...
  async startShift(context: ShiftChatroomContext): Promise<{
    ...
    try {
      ...
      }).catch((err: unknown) => log.warn('[ShiftChatroomWorkflow] publish shift_started failed:', err.message));
  ...
  async endShift(
    ...
    try {
      ...
      }).catch((err: unknown) => log.warn('[ShiftChatroomWorkflow] publish shift_completed failed:', err.message));
      ...
      if (darResult.darId) {
        ...
        }).catch((err: unknown) => log.warn('[ShiftChatroom] dar_submitted publish failed (non-blocking):', err.message));
      }
  ...
  private async generateDAR(
    ...
    }).catch((err: unknown) => log.warn('[ShiftChatroomWorkflow] publish dar_generated failed:', err.message));
