...
class ShiftChatroomWorkflowService {
  ...
  async startShift(context: ShiftChatroomContext): Promise<{
    ...
    try {
      ...
      }).catch((err: unknown) => log.warn('[ShiftChatroomWorkflow] publish shift_started failed:', err instanceof Error ? err.message : String(err)));
      ...
      (async () => {
        try {
          ...
          log.warn('[ShiftChatroomWorkflow] ReportBot welcome failed (non-blocking):', botErr instanceof Error ? botErr.message : String(botErr));
        }
      })();
  ...
  async endShift(
    ...
    try {
      ...
      }).catch((err: unknown) => log.warn('[ShiftChatroomWorkflow] publish shift_completed failed:', err instanceof Error ? err.message : String(err)));
      ...
      if (darResult.darId) {
        try {
          ...
          log.error(`[ShiftChatroom] Auto PDF generation failed (non-blocking):`, pdfErr instanceof Error ? pdfErr.message : String(pdfErr));
        }
        ...
        }).catch((err: unknown) => log.warn('[ShiftChatroom] dar_submitted publish failed (non-blocking):', err instanceof Error ? err.message : String(err)));
      }
      ...
      if (message.messageType !== 'system' && userId !== 'reportbot') {
        (async () => {
          try {
            ...
            log.warn('[ShiftChatroomWorkflow] Bot processing failed (non-blocking):', botErr instanceof Error ? botErr.message : String(botErr));
          }
        })();
      }
  ...
  private async generateDAR(
    ...
    }).catch((err: unknown) => log.warn('[ShiftChatroomWorkflow] publish dar_generated failed:', err instanceof Error ? err.message : String(err)));
    ...
    if (officerMessages.length === 0) {
      ...
      for (const msg of officerMessages) {
        ...
        if (msg.messageType === 'photo') {
          const gps = (msg as {metadata: Record<string, unknown>}).metadata?.gps;
