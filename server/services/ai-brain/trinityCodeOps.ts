...
class TrinityCodeOpsService {
  ...
  async searchCode(request: CodeSearchRequest): Promise<CodeSearchResponse> {
    ...
    try {
      ...
      console.error('[TrinityCodeOps] Search error:', error.message);
      return {
        success: false,
  ...
  async applyPatch(request: PatchRequest): Promise<PatchResult> {
    ...
    console.log(`[TrinityCodeOps] Applying ${patches.length} patches, operation: ${operationId}`);
    ...
    try {
      ...
      if (requiresApproval) {
        ...
        if (riskLevel === 'high' || riskLevel === 'critical') {
          ...
          platformEventBus.publish({
            ...
            data: { approvalId: approval.id, operationId, workspaceId, riskLevel, patchCount: patches.length },
      ...
      for (const patch of patches) {
        try {
          ...
          errors.push(`Failed to apply patch to ${patch.file}: ${err.message}`);
          failedPatches++;
        }
      }
      ...
      platformEventBus.publish({
        ...
        data: { operationId, workspaceId, appliedPatches, commitHash },
        severity: 'info',
        isNew: true
      });
      ...
      console.error('[TrinityCodeOps] Patch application error:', error);
  ...
  async commitChanges(request: GitCommitRequest): Promise<GitCommitResult> {
    ...
    try {
      ...
      console.error('[TrinityCodeOps] Git commit error:', error);
      return {
        success: false,
        filesChanged: 0,
        insertions: 0,
  ...
  async getDiff(files?: string[]): Promise<string> {
    try {
      ...
      console.error('[TrinityCodeOps] Git diff error:', error);
      return '';
    }
  }
  ...
  async rollbackOperation(operationId: string): Promise<boolean> {
    ...
    if (!history) {
      console.warn(`[TrinityCodeOps] No history found for operation: ${operationId}`);
      return false;
    }
    ...
    try {
      ...
      console.error('[TrinityCodeOps] Rollback error:', error);
      return false;
    }
  }
  ...
  private async logOperation(type: string, data: Record<string, any>): Promise<void> {
    try {
      ...
      console.error('[TrinityCodeOps] Failed to log operation:', error);
    }
  }
  ...
  async executeFastMode(operations: Array<{ type: 'search' | 'patch' | 'read'; params: any }>): Promise<any[]> {
    console.log(`[TrinityCodeOps] FAST MODE: Executing ${operations.length} operations in parallel`);
    const startTime = Date.now();
    ...
    console.log(`[TrinityCodeOps] FAST MODE completed in ${totalTime}ms`);
