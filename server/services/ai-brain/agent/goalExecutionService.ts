...
export const goalExecutionService = {
  async execute(workspaceId: string, goalType: GoalType, params: Record<string, unknown> = {}): Promise<GoalResult> {
    ...
    try {
      ...
      await pool.query(`UPDATE agent_tasks SET status='failed', error_message=$1 WHERE id=$2`, [(err instanceof Error ? err.message : String(err)), taskId]).catch(() => {});
      log.error(`[GoalExecution] ${goalType} failed: ${(err instanceof Error ? err.message : String(err))}`);
      return { goalType, success: false, actionsExecuted: 0, details: (err instanceof Error ? err.message : String(err)) };
    }
  },
};
