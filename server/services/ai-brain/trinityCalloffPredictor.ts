...
class TrinityCalloffPredictor {
  ...
  private async computeCalloffRisks(workspaceId: string, horizonHours: number): Promise<CalloffRisk[]> {
    try {
      ...
      return (result as unknown[]).map(row => this.scoreCalloffRisk(row));
    } catch (err: unknown) {
  ...
  private async computeSiteRisks(workspaceId: string): Promise<SiteRiskScore[]> {
    try {
      ...
      return (result as unknown[]).map(row => this.scoreSiteRisk(row));
    } catch (err: unknown) {
  ...
  async getOfficerCalloffRisk(employeeId: string, workspaceId: string): Promise<{
    ...
    try {
      ...
      if (!(result as unknown[]).length) return null;

      const row = { ...(result as unknown[])[0], day_of_week: new Date().getDay(), shift_hour: 8 };
