...
class TrinitySelfEditGovernanceService {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      console.log('[Trinity Self-Edit] Governance service initialized with database persistence');
    } catch (error) {
      console.error('[Trinity Self-Edit] Failed to initialize from database:', error);
    }
  }
  ...
  private async loadCircuitBreakerState(): Promise<void> {
    try {
      ...
      const recentChanges = await db.select()
        ...
        .where(and(
          eq(automationActionLedger.actionType, 'trinity_self_edit'),
      ...
      const appliedChanges = recentChanges.filter(c => 
        c.status === 'applied' || c.status === 'failed'
      );
      const failures = appliedChanges.filter(c => c.status === 'failed');
      this.circuitBreakerState.errorRate = appliedChanges.length > 0
        ? failures.length / appliedChanges.length
        : 0;
      
      console.log(`[Trinity Self-Edit] Circuit breaker state loaded: ${changesLastHour.length}/hr, ${recentChanges.length}/day`);
    } catch (error) {
      console.error('[Trinity Self-Edit] Failed to load circuit breaker state:', error);
    }
  }
  ...
  private tripCircuitBreaker(reason: string): void {
    ...
    console.warn(`🛑 [Trinity Self-Edit] Circuit breaker tripped: ${reason}`);
  }
  ...
  private async logProposal(proposal: ChangeProposal): Promise<void> {
    try {
      ...
      await db.insert(automationActionLedger).values({
        id: proposal.id,
        actionType: 'trinity_self_edit',
        status: proposal.status === 'pending' ? 'pending_approval' : proposal.status as any,
        workspaceId: proposal.workspaceId || null,
        initiatedBy: 'trinity_ai',
        metadata: {
          ...
          reasoning: proposal.reasoning,
          // Store full code payload for restart recovery
      ...
      console.log(`[Trinity Self-Edit] Proposal ${proposal.id} persisted to database`);
    } catch (error) {
      console.error('[Trinity Self-Edit] Failed to log proposal:', error);
    }
  }
  ...
  private async updateProposalStatus(proposalId: string, status: string, additionalMeta?: Record<string, any>): Promise<void> {
    try {
      ...
      if (existing.length === 0) {
        console.warn(`[Trinity Self-Edit] Proposal ${proposalId} not found in database`);
        return;
      }
      ...
      console.log(`[Trinity Self-Edit] Updated proposal ${proposalId} status to ${status}`);
    } catch (error) {
      console.error('[Trinity Self-Edit] Failed to update proposal status:', error);
    }
  }
  ...
  async loadProposalsFromDatabase(): Promise<void> {
    try {
      ...
      const proposals = await db.select()
        ...
        .where(and(
          eq(automationActionLedger.actionType, 'trinity_self_edit'),
          sql`status IN ('pending_approval', 'approved', 'auto_approved', 'applying')`
        ));
      ...
      for (const p of proposals) {
        ...
        if (meta?.type === 'self_edit_proposal') {
          ...
          if (dbStatus === 'approved') {
            ...
            needsRecovery = true;
            console.warn(`[Trinity Self-Edit] Proposal ${p.id} was interrupted mid-apply, marking for recovery`);
      ...
      console.log(`[Trinity Self-Edit] Loaded ${proposals.length} proposals from database (${pendingCount} pending, ${approvedCount} approved awaiting application)`);
    } catch (error) {
      console.error('[Trinity Self-Edit] Failed to load proposals from database:', error);
    }
  }
  ...
  private async recoverInterruptedProposal(proposal: ChangeProposal): Promise<void> {
    try {
      console.log(`[Trinity Self-Edit] Recovering interrupted proposal ${proposal.id}`);
      ...
      if (proposal.rollbackHash) {
        try {
          ...
          console.log(`[Trinity Self-Edit] Rolled back to ${proposal.rollbackHash} for proposal ${proposal.id}`);
        } catch (rollbackError) {
          console.warn(`[Trinity Self-Edit] Rollback failed for ${proposal.id}, manual intervention may be needed:`, rollbackError);
        }
      }
      ...
      console.error(`[Trinity Self-Edit] Failed to recover proposal ${proposal.id}:`, error);
  ...
  async applyApprovedChanges(proposalId: string): Promise<{ success: boolean; commitHash?: string; error?: string }> {
    ...
    try {
      ...
      this.circuitBreakerState.lastError = error.message;
      ...
      case 'modify':
        if (change.newContent) {
          ...
          if (fs.existsSync(change.file)) {
            ...
            if (currentHash === targetHash) {
              console.log(`[Trinity Self-Edit] Skipping ${change.file} - already has target content (idempotent)`);
              return;
            }
...
${proposal.reasoning}`;
      ...
      console.warn('[Trinity Self-Edit] Git commit skipped:', error.message);
  ...
  private async rollbackToCommit(commitHash: string): Promise<void> {
    try {
      ...
      console.error('[Trinity Self-Edit] Rollback failed:', error);
      throw error;
    }
  }
