...
async function getTrendData(workspaceId: string, range: DateRange): Promise<TrendData[]> {
  ...
  return ((trendQuery as Record<string,unknown>).rows as any[]).map(row => ({
    period: row.period,
...
export async function getTimeUsageMetrics(
  ...
  return {
    ...
    byEmployee: ((byEmployee as Record<string,unknown>).rows as any[]).map(row => ({
    ...
    byClient: ((byClient as Record<string,unknown>).rows as any[]).map(row => ({
      clientId: row.client_id,
      name: row.name || 'Unknown',
      totalHours: parseFloat(row.total_hours) || 0,
      revenue: parseFloat(row.revenue) || 0
    })),
    byDay: ((byDay as Record<string,unknown>).rows as any[]).map(row => ({
      date: row.date,
      hours: parseFloat(row.hours) || 0,
...
export async function getSchedulingMetrics(
  ...
  return {
    ...
    byStatus: ((statusData as Record<string,unknown>).rows as any[]).map(row => ({
      status: row.status,
      count: parseInt(row.count)
    })),
    byDay: ((dailyData as Record<string,unknown>).rows as any[]).map(row => ({
...
export async function getRevenueMetrics(
  ...
  return {
    ...
    byMonth: ((byMonth as Record<string,unknown>).rows as any[]).map(row => ({
      month: row.month,
      invoiced: parseFloat(row.invoiced) || 0,
      paid: parseFloat(row.paid) || 0
    })),
...
export async function getEmployeePerformanceMetrics(
  ...
  const employees: EmployeePerformance[] = ((performanceData as Record<string,unknown>).rows as any[]).map(row => ({
    employeeId: row.employee_id,
    name: row.name || 'Unknown',
