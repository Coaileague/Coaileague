...
async function phase1_auth_system_completeness() {
  ...
  `);
  const userCols = (usersTbl as unknown[]).map((r: unknown) => r.column_name) || [];
...
async function phase2_crud_entity_schemas() {
  ...
  for (const entity of coreEntities) {
    ...
    `);
    const cols = (colResult as unknown[]).map((r: unknown) => r.column_name) || [];
...
async function phase3_workspace_isolation() {
  ...
  const dataTablesWithWorkspaceId = await typedQuery(sql`
    ...
    ORDER BY table_name
  `);
  const wsTableCount = (dataTablesWithWorkspaceId as unknown[]).length || 0;
...
async function phase4_stripe_webhook_handlers() {
  ...
  `);
  const peColumns = (processedEventsTable as unknown[]).map((r: unknown) => r.column_name) || [];
  record({
...
async function phase10_database_referential_integrity() {
  ...
  `);
  const wsIdxCount = (wsIdxResult as unknown[]).length || 0;
  record({
