...
import { db } from './db';
import { employees, achievements, employeePoints } from '@shared/schema';
...
gamificationRouter.post('/achievements', requireWorkspaceRole(['org_owner', 'org_admin']), async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    res.status(400).json({ error: error.message || 'Failed to create achievement' });
  }
});
...
gamificationRouter.post('/award', requireWorkspaceRole(['org_owner', 'org_admin', 'department_manager']), async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    res.status(400).json({ error: error.message || 'Failed to award' });
  }
});
 ...
 */
gamificationRouter.get('/employees/:id', requireWorkspaceRole(['org_owner', 'org_admin', 'department_manager']), async (req: AuthenticatedRequest, res) => {
  try {
    const user = req.user;
    if (!user?.currentWorkspaceId) {
      return res.status(400).json({ error: 'No workspace selected' });
    ...
    console.error('Error fetching employee gamification:', error);
