...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/feature-updates/clear-all', requireAuth, async (req: AuthenticatedRequest, res) => {
    try {
      ...
      const dismissedReceipts = await db
        .select({ featureUpdateId: featureUpdateReceipts.featureUpdateId })
        .from(featureUpdateReceipts)
        .where(
          and(
            eq(featureUpdateReceipts.userId, userId),
      ...
      // Dismiss all undismissed updates
      for (const update of activeUpdates) {
        if (!dismissedIds.has(update.id)) {
          ...
          const existingReceipt = await db
            ...
            .where(
              and(
                eq(featureUpdateReceipts.userId, userId),
                eq(featureUpdateReceipts.featureUpdateId, update.id)
              )
            )
            .limit(1);
