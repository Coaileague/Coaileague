...
staffingBroadcastRouter.post('/broadcast', requireAuth, async (req: Request, res: Response) => {
  const user = req.user;
  const workspaceId = req.workspaceId || user?.currentWorkspaceId || (user as Record<string,unknown>)?.workspaceId;
  ...
  try {
    const result = await createShiftBroadcast({
      workspaceId,
      broadcastedBy: user?.id || (user as Record<string,unknown>)?.claims?.sub || 'system',
      ...parsed.data,
    });
