...
router.patch("/proposals/:id", requireAuth, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    if (req.body.stage === 'won') {
      platformEventBus.publish({ type: 'proposal_won', category: 'automation', title: 'Proposal Won', description: `Deal ${req.params.id} marked as won.`, workspaceId: wid, metadata: { dealId: req.params.id } }).catch((err: unknown) => log.warn('[EventBus] Publish failed (non-blocking):', (err instanceof Error ? err.message : String(err))));
    } else if (req.body.stage === 'lost') {
      platformEventBus.publish({ type: 'proposal_lost', category: 'automation', title: 'Proposal Lost', description: `Deal ${req.params.id} marked as lost. Reason: ${req.body.loss_reason || 'Not specified'}`, workspaceId: wid, metadata: { dealId: req.params.id, reason: req.body.loss_reason } }).catch((err: unknown) => log.warn('[EventBus] Publish failed (non-blocking):', (err instanceof Error ? err.message : String(err))));
    } else if (req.body.stage === 'proposal' || req.body.stage === 'rfp') {
      platformEventBus.publish({ type: 'bid_submitted', category: 'automation', title: 'Bid/Proposal Submitted', description: `Deal ${req.params.id} moved to ${req.body.stage} stage.`, workspaceId: wid, metadata: { dealId: req.params.id } }).catch((err: unknown) => log.warn('[EventBus] Publish failed (non-blocking):', (err instanceof Error ? err.message : String(err))));
    }
