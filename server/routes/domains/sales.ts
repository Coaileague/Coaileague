...
export function mountSalesRoutes(app: Express): void {
  ...
  app.post("/api/ethics/report", (req: AuthenticatedRequest, res: Response, next: NextFunction) => rfpEthicsRouter(req, res, next));
  app.get("/api/ethics/followup/:token", (req: AuthenticatedRequest, res: Response, next: NextFunction) => rfpEthicsRouter(req, res, next));
