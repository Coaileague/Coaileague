...
router.post("/:approvalId/decide", requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    if (decision === 'approved' && updated.employeeId) {
      ...
      platformEventBus.publish({ type: 'compliance_document_approved', category: 'automation', title: 'Compliance Document Approved', description: `Document '${decisionNotes || 'Compliance Document'}' approved for employee ${updated.employeeId}`, workspaceId, metadata: approvedPayload }).catch((err: unknown) => log.warn('[EventBus] Publish failed (non-blocking):', (err instanceof Error ? err.message : String(err))));
      ...
      platformEventBus.publish({ type: 'compliance_document_rejected', category: 'automation', title: 'Compliance Document Rejected', description: `Document '${decisionNotes || 'Compliance Document'}' rejected for employee ${updated.employeeId}. Reason: ${decisionNotes || 'N/A'}`, workspaceId, metadata: rejectedPayload }).catch((err: unknown) => log.warn('[EventBus] Publish failed (non-blocking):', (err instanceof Error ? err.message : String(err))));
