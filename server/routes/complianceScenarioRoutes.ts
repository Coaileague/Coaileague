...
router.post('/confirm-renewal/:employeeId', requireAuth, async (req, res) => {
  try {
    ...
    await createNotification({
      ...
      idempotencyKey: `compliance_alert-${Math.floor(Date.now() / (6 * 60 * 60 * 1000))}-${confirmedByUserId}`,
...
router.post('/override-out-of-state/:employeeId', requireAuth, async (req, res) => {
  try {
    ...
    await createNotification({
      ...
      idempotencyKey: `compliance_alert-${Math.floor(Date.now() / (6 * 60 * 60 * 1000))}-${approvedByUserId}`,
