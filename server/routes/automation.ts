...
automationRouter.post('/schedule/generate', requireAuth, async (req: any, res: Response) => {
  try {
    ...
    const employees = await storage.getEmployeesByWorkspace((req.workspaceId || (req.user as any)?.currentWorkspaceId));
    
    // Get existing shifts in date range to avoid conflicts
    const existingShifts = await storage.getShiftsByWorkspace(
      (req.workspaceId || (req.user as any)?.currentWorkspaceId),
      new Date(startDate),
      new Date(endDate)
    );
    ...
    try {
      ...
      broadcastToWorkspace((req.workspaceId || (req.user as any)?.currentWorkspaceId), { type: 'schedules_updated' });
    } catch (e: unknown) { log.warn('[Automation] Broadcast failed:', e.message); }

    const _wsId1 = req.workspaceId || (req.user as any)?.currentWorkspaceId;
...
automationRouter.post('/invoice/generate', requireAuth, async (req: any, res: Response) => {
  try {
    ...
    // Get client
    const client = await storage.getClient(clientId, (req.workspaceId || (req.user as any)?.currentWorkspaceId));
    if (!client || client.workspaceId !== (req.workspaceId || (req.user as any)?.currentWorkspaceId)) {
      return res.status(404).json({ error: 'Client not found' });
    }

    // Get unbilled time entries for this client
    const timeEntries = await storage.getUnbilledTimeEntries((req.workspaceId || (req.user as any)?.currentWorkspaceId), clientId);
    ...
    try {
      const { broadcastToWorkspace } = await import('../websocket');
      broadcastToWorkspace((req.workspaceId || (req.user as any)?.currentWorkspaceId), { type: 'invoices_updated' });
    } catch (e: unknown) { log.warn('[Automation] Broadcast failed:', e.message); }

    const _wsId2 = req.workspaceId || (req.user as any)?.currentWorkspaceId;
    platformEventBus.publish({
...
automationRouter.post('/invoice/anchor-close', requireAuth, async (req: any, res: Response) => {
  try {
    ...
    if (result.invoices.length > 0) {
      try {
        const orgLeaders = await db.select()
          ...
          .where(
            and(
              eq(employees.workspaceId, (req.workspaceId || (req.user as any)?.currentWorkspaceId)),
        ...
        for (const leader of orgLeaders) {
          if (leader.userId) {
            await createNotification({
              ...
              relatedEntityId: (req.workspaceId || (req.user as any)?.currentWorkspaceId),
              metadata: { 
    ...
    const _wsId3 = req.workspaceId || (req.user as any)?.currentWorkspaceId;
    platformEventBus.publish({
      type: 'invoice_created',
      category: 'automation',
...
automationRouter.post('/payroll/generate', requireAuth, async (req: any, res: Response) => {
  try {
    ...
    // Get employee
    const employee = await storage.getEmployee(employeeId, (req.workspaceId || (req.user as any)?.currentWorkspaceId));
    if (!employee || employee.workspaceId !== (req.workspaceId || (req.user as any)?.currentWorkspaceId)) {
    ...
    try {
      const { broadcastToWorkspace } = await import('../websocket');
      broadcastToWorkspace((req.workspaceId || (req.user as any)?.currentWorkspaceId), { type: 'payroll_updated' });
    } catch (e: unknown) { log.warn('[Automation] Broadcast failed:', e.message); }

    const _wsId4 = req.workspaceId || (req.user as any)?.currentWorkspaceId;
    platformEventBus.publish({
...
automationRouter.post('/payroll/anchor-close', requireAuth, async (req: any, res: Response) => {
  try {
    ...
    const _wsId5 = req.workspaceId || (req.user as any)?.currentWorkspaceId;
    platformEventBus.publish({
      type: 'payroll_run_created',
      category: 'automation',
...
automationRouter.post('/compliance/scan', requireAuth, async (req: any, res: Response) => {
  try {
    ...
    const creditResult = await withCredits(
      ...
      async () => {
        return await ComplianceMonitoringService.scanWorkspace((req.workspaceId || (req.user as any)?.currentWorkspaceId));
      }
    );
    ...
    await storage.createAuditEvent({
      ...
      aggregateId: (req.workspaceId || (req.user as any)?.currentWorkspaceId),
...
automationRouter.post('/photo/validate', requireAuth, async (req: any, res: Response) => {
  try {
    ...
    const employee = await storage.getEmployeeByUserId(req.user?.id, (req.workspaceId || (req.user as any)?.currentWorkspaceId));
    if (!employee) {
...
automationRouter.get('/quickbooks/stats', async (req: any, res: Response) => {
  try {
    ...
    const stats = await quickbooksReceiptService.getSyncStats((req.workspaceId || (req.user as any)?.currentWorkspaceId));
...
automationRouter.get('/trinity/settings', async (req: any, res: Response) => {
  try {
    ...
    const settings = await trinityAutomationToggle.getSettings((req.workspaceId || (req.user as any)?.currentWorkspaceId));
    return res.json({ settings });
  } catch (error) {
