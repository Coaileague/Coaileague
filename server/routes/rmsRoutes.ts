...
rmsRouter.post("/dars/:id/send-to-client", requireAuth as any, ensureWorkspaceAccess as any, async (req: any, res: any) => {
  try {
    ...
    if (recipientEmail) {
      try {
        ...
        await NotificationDeliveryService.send({ idempotencyKey: `notif-${Date.now()}`,
            type: 'report_delivery', workspaceId: workspaceId || 'system', recipientUserId: recipientEmail, channel: 'email', body: { to: recipientEmail, subject: `Daily Activity Report - ${dar.site_name || 'Site'} - ${dar.shift_date || new Date().toLocaleDateString()}`, html: darHtml } });
    ...
    platformEventBus.publish({ idempotencyKey: `notif-${Date.now()}`,
            type: 'dar_sent_to_client', category: 'automation', title: `DAR Sent to Client — ${dar.report_number}`, description: `${dar.report_number} delivered to ${recipientEmail || 'client'}`, workspaceId, metadata: { darId: req.params.id, reportNumber: dar.report_number, recipientEmail } }).catch((err: any) => log.warn('[EventBus] Publish failed (non-blocking):', err?.message));
