    ...
    try {
      ...
      if (autoSend && dar.client_email) {
        ...
        await NotificationDeliveryService.send({
          idempotencyKey: `notif:dar:${req.params.id}:dar_delivered`,
            type: 'dar_delivered',
          workspaceId,
  ...
  try {
    ...
    if (recipientEmail) {
      try {
        ...
        await NotificationDeliveryService.send({ idempotencyKey: `notif:dar:${req.params.id}:report_delivery`,
    ...
    platformEventBus.publish({ idempotencyKey: `notif:dar:${req.params.id}:dar_sent_to_client`,
