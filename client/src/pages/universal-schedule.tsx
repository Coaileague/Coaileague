...
export default function UniversalSchedule({ defaultViewMode }: { defaultViewMode?: 'my' | 'full' | 'pending' } = {}) {
  ...
  const { data: allShiftsData = [], isLoading: allShiftsLoading } = useQuery<Shift[]>({
    ...
    queryFn: async () => {
      const response = await secureFetch(`/api/shifts?workspaceId=${workspaceId}`, { credentials: 'include' });
  ...
  const { data: employees = [], isLoading: employeesLoading } = useQuery<{ data: Employee[] }, Error, Employee[]>({
    ...
    queryFn: async () => {
      ...
      const res = await secureFetch(`/api/employees?workspaceId=${workspaceId}&limit=500`, { credentials: 'include' });
  ...
  const handleToolbarPublish = useCallback(async () => {
    ...
    try {
      ...
      toast({ variant: 'destructive', title: 'Publish Failed', description: (error instanceof Error ? error.message : String(error)) });
    }
