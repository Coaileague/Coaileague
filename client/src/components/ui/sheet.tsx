...
const sheetVariants = cva(
  "fixed z-50 gap-3 bg-background p-3 md:p-4 shadow-lg transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500",
  {
    variants: {
      side: {
...
const SheetContent = React.forwardRef<
  ...
  SheetContentProps
>(({ side = "right", className, children, showHomeButton = HOME_BUTTON_CONFIG.enabled, homeButtonPath, isGuest = false, ...props }, ref) => {
    ...
    <SheetPortal>
      ...
      >
        ...
        <div className="absolute right-2 top-2 flex items-center gap-0.5">
          ...
          <SheetPrimitive.Close className="flex items-center justify-center rounded-md min-h-11 min-w-11 opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-secondary hover:bg-accent active:bg-accent/80">
            <X className="h-5 w-5" />
