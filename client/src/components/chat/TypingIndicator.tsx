...
export function TypingIndicator({ userName, isStaff = false, viewerIsStaff = false }: TypingIndicatorProps) {
  const displayText = isStaff 
    ? "Agent is typing" 
    : viewerIsStaff 
      ? "User is typing" 
      : `${userName} is typing`;
  ...
  return (
    <div className="flex items-center gap-2 px-4 py-2 mb-2" data-testid="div-typing-indicator">
      <div className="flex items-center gap-1.5 min-w-0">
        <span className="text-xs font-medium text-slate-600 dark:text-slate-400 truncate" data-testid="text-typing-user">
          {displayText}
        </span>
        {isStaff && (
          <Badge 
            variant="secondary" 
            className="h-4 px-1.5 text-[10px] bg-emerald-100 dark:bg-emerald-900 text-blue-700 dark:text-emerald-300 border-emerald-200 dark:border-emerald-800 shrink-0"
      ...
      <div className="flex items-center gap-1 shrink-0" data-testid="div-typing-dots">
